  ...
  <n-grid n-grid cols="1 620:3" :x-gap="12" :y-gap="8" item-responsive >
    <n-gi :span="2">
      <n-card title="最新版本">
        ...
        <n-button @click="checkUpdate" type="primary" style="width: 100px">检查更新</n-button>
    ...
    <n-gi>
      <n-card title="更新记录">
        <n-timeline v-for="log in updateLogs" :key="log.filename" >
          <n-timeline-item :type="log.type" :title="log.name" :content="log.des" :time="log.time" line-type="default"/>
        </n-timeline>
      </n-card>
    </n-gi>
  </n-grid>
...
export default defineComponent({
    ...
    mounted() {
        this.$axios.get('https://storage.rmdarck.icu/louise/louise-version.json').then(result => {
    ...
    methods: {
      checkUpdate() {
        this.$axios.get('https://storage.rmdarck.icu/louise/DEVELOP.md').then(result => {
